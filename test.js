  ...
  cloud.getSensors(function(err, sensors) {
    ...
    f = function(offset, p, s) {
      return function(err, sensor) {
        ...
        if (!!err) return console.log(s + ' id=' + p.id + ': ' +
          err.message);

        console.log('sensor #' + offset + ' ' + s + ': ');
        console.log(sensor);
        props = {
          temp: ['temperature', 'celcius', 'meteo'],
        ...
        console.log('/device/climate/' + (sensor.protocol ||
          'telldus') + '/' + type[2]);
        console.log('    uuid=teldus:' + sensor.id);
        console.log('    name: ' + sensor.name);
        console.log('    status: ' + (p.online === '1' ? 'present' :
          'absent'));
        ...
        for (i = 0; i < sensor.data.length; i++) {
          prop = props[sensor.data[i].name];
          if (prop) console.log('      ' + prop[0] + ': ' + sensor.data[
            i].value);
        }
        console.log('');
      };
    ...
    for (i = 0; i < sensors.length; i++) cloud.getSensorInfo(sensors[i],
      f(i, sensors[i], 'getSensorInfo'));
    ...
    /*
        ...
        f = function(offset, p, s) {
          return function(err, device) {
            ...
            console.log('device #' + offset + ' ' + s + ': ');
            console.log(device);
            types = {
              'selflearning-switch': 'onoff',
            ...
            console.log('/device/switch' + '/' + (d[d.length - 1] ||
              'telldus') + '/' + type);
            console.log('    uuid=teldus:' + device.id);
            console.log('    perform: off, on');
            console.log('    name: ' + device.name);
            console.log('    status: ' + (device.online === '0' ?
              'absent' : device.status));
            console.log('    info:');
            if (type === 'dimmer') console.log(
              '      dimmer: percentage');
            console.log('    values:');
            if (type === 'dimmer') console.log('      dimmer: ' + Math.round(
              (1 - (255 - device.statevalue) / 255) * 100) + '%');
            console.log('');
          };
        };

        for (i = 0; i < devices.length; i++) {
          if (devices[i].type === 'device') cloud.getDeviceInfo(devices[i],
            f(i, devices[i], 'getDeviceInfo'));
        }
        */
  });
